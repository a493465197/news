...
class MongoPipeline(object):
    ...
    def close_spider(self, spider):
        ...
        self.db['run'].update_one({'id1': spider.id1}, {'$set':{'count': count}})
        self.client.close()

    def process_item(self, item, spider):
        self.count = self.count+1
        if isinstance(item, NewsinaspiderItem):
            self.insert_item(self.db[item.collection], item, spider)
        return item
    ...
    def insert_item(collection, item, spider):
        ...
        try:
            v = dict(item)
            v['runId1'] = spider.id1
            collection.update_one({'title': v.get('title')}, {'$set':v},upsert=True)
